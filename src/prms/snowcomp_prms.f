...
! PRMS_SNOW module for defining stateful variables
      ...
      REAL, SAVE :: Basin_snowmelt, Basin_pweqv, Basin_snowdepth
      REAL, SAVE :: Basin_snowcov, Basin_snowevap, Basin_pk_precip
...
!***********************************************************************
      INTEGER FUNCTION snowcomp_prms()
...
!***********************************************************************
      ...
      IF ( declmodule(
     +'$Id: snowcomp_prms.f 3116 2011-05-17 16:20:01Z rsregan $'
     +).NE.0 ) RETURN
...
! declare variables
      IF ( declvar('snow', 'basin_snowdepth', 'one', 1, 'real',
     +     'Basin area-weighted average snow depth',
     +     'inches',
     +     Basin_snowdepth).NE.0 ) RETURN
...
!***********************************************************************
      ...
      USE PRMS_SNOW
      USE PRMS_BASIN, ONLY: Nhru, NEARZERO, Timestep, Print_debug
...
!***********************************************************************
     ...
     +    Basin_area_inv, Hru_route_order, Nhru, Print_debug
...
!         CALL dpreal('tcal-alb', Tcal(i), 1, 1)
          ...
          cec = Cecn_coef(Nowmonth)*.5 ! [cal/(cm^2 degC)] 
                                 ! or [Langleys / degC]
...
!         CALL dpreal('tcal-nite', Tcal(i), 1, 1)
          ...
          IF ( Pkwater_equiv(i).GT.0. ) THEN
            ...
            ! temp for the day
            temp = (Tmaxc(i)+Tavgc(i))*.5 ! [degrees C]
            CALL snowbal(niteda, Tstorm_mo(Nowmonth), Iasw(i),
     +                   temp, esv, Basin_ppt, trd, Emis_noppt,
...
!           CALL dpreal('tcal-day', Tcal(i), 1, 1)
     ...
     +           (Transp_on(i).EQ.1 .AND. Cov_type(i)<2) )
...
! LAST check to clear out all arrays if packwater is gone
     ...
     +             Newsnow(i), Pptmix(i), Pptmix_nopack(i)
      ...
      IF ( Print_debug.EQ.1 ) THEN
        ...
        ELSEIF ( ABS(bsnobal).GT.5.0E-5 ) THEN
...
!***********************************************************************
     ...
     +                 Pk_depth, Pss, Pst, Iasw, Pk_den)
      ...
      USE PRMS_BASIN, ONLY: NEARZERO, Print_debug
...
! Arguments
      ...
      REAL, INTENT(IN) :: Emis_noppt, Basin_ppt, Sw
      ...
      IF ( Basin_ppt.GT.0. ) THEN
        IF ( Tstorm_mo.EQ.1 ) THEN
          ...
          IF ( Niteda.EQ.1 ) THEN
            ...
            IF ( Trd.GT.ONETHIRD ) emis = Emis_noppt ![fraction of radiation]
          ...
          ELSE
            ...
            ! 0.85 and 0.75
            IF ( Trd.GT.ONETHIRD ) emis = 1.29 - (.882*Trd)
...
!***********************************************************************
      ...
      ai = Pst ! [inches]
      IF ( ai.GT.Snarea_thresh ) ai = Snarea_thresh ! [inches]
