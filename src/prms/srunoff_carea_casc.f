...
!***********************************************************************
      ...
      IF ( declmodule(
     +'$Id: srunoff_carea_casc.f 3116 2011-05-17 16:20:01Z rsregan $'
     +).NE.0 ) RETURN
...
!***********************************************************************
      SUBROUTINE compute_infil_casc(Pptmix_nopack, Carea_min,
     ...
     +           Upslope_hortonian)
      USE PRMS_SRUNOFF_CAREA_CASC, ONLY: Srp, Sri
...
!***********************************************************************
      IF ( Hru_type==1 ) THEN
        ...
        IF ( ca_percent>1.0 ) ca_percent = 1.0
        Srp = ca_percent*Pptp
        IF ( Srp<0.0 ) Srp = 0.0
        Infil = Pptp - Srp
      ELSE ! Hru_type=3
