...
support is provided for compilation. An executable file is not provided.
...
The following commands can be used to extract GSFLOW:
    ...
    gunzip gsflow_1.1.6.tar.gz
    tar -xvf gsflow_1.1.6.tar

The tar command will extract the directory GSFLOW_1.1.6 that contains
...
directory:
   ...
   |
   |--GSFLOW_1.1.6
   |    |--bin           ; Empty. The Makefile is configured to place a compiled executable here
   |    |--lib           ; Empty. The Makefile is configured to place compiled libraries here
   |    |--data
...
should be included in the PATH environment variable found in your
...
After the executable file is installed in the GSFLOW_1.1.6/bin directory
...
An executable file is not provided in the GSFLOW_1.1.6/bin subdirectory.
...
command make in the GSFLOW_1.1.6/src subdirectory. However, the USGS
